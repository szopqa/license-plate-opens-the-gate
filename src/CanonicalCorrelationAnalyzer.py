...
class CanonicalCorrelationAnalyzer():
  ...
  """
  def __init__(self, reader, licensePlateValidator):
    self._licensePlateValidator = licensePlateValidator
    self._reader = reader
    (
      self._labeled_image, 
      num_of_labels
    ) = measure.label(self._reader.binary_image, return_num=True, connectivity=2)
  ...
  def get_plate_like_objects(self):
    for each_region in regionprops(self._labeled_image):
      ...
      if self._licensePlateValidator.validate_width_and_height(region_width, region_height):
        self._plate_like_objects.append(self._reader.binary_image[min_row:max_row, min_col:max_col])
        ...
        ax1.imshow(self._reader.binary_image, cmap="gray")
        # ax2.imshow(self._labeled_image)
        ax2.imshow(self._reader.get_binary_resized(0.6))
